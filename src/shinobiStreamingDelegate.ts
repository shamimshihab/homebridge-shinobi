...
export class ShinobiStreamingDelegate implements CameraStreamingDelegate {
  ...
  constructor(
    ...
    this.platform.log.info(
      `creating ShinobiStreamingDelegate using shinobi config: ${JSON.stringify(
        shinobiConfig
      )}`
    );
    ...
    let monitorDetails;
    // this.platform.log.info(
    //     `Type of shinobiConfig.details: ${typeof shinobiConfig.details}`
    //   );
    try {
      if (typeof shinobiConfig.details === "string") {
        monitorDetails = JSON.parse(shinobiConfig.details);
        this.platform.log.warn("Succeeded in parsing JSON.");
      } else {
      ...
      this.platform.log.warn(
        "Failed to parse shinobiConfig.details as JSON, using raw value."
      );
    }
    ...
    if (this.monitor.useSubStream) {
      if (
        monitorDetails.substream &&
        monitorDetails.substream.input &&
        monitorDetails.substream.input.fulladdress
      ) {
        ...
        this.platform.log.info(
          `ShinobiStreamingDelegate using shinobi dynamic substream proxy source: ${this.videoSource}`
        );
      }
    } else {
      if (monitorDetails.auto_host) {
        ...
        this.platform.log.info(
          `ShinobiStreamingDelegate using direct camera source: ${this.videoSource}`
        );
      } else {
        this.monitor.proxyStream = true;
        this.platform.log.info(
          `ShinobiStreamingDelegate using shinobi proxy source: ${this.videoSource}`
        );
      }
    }
  }
  ...
  handleSnapshotRequest(
    ...
    this.platform.log.debug(
      "handleSnapshotRequest: " +
        `${this.monitor.monitorConfig.monitor_id} => ${JSON.stringify(
          request
        )} from ${this.imageSource}`
    );
    ...
    fetch(this.imageSource)
      ...
      .catch((err) => {
        this.platform.log.error(
          `handleSnapshotRequest() error: ${err.message}`
        );
        callback(err);
      });
  }
  ...
  async prepareStream(
    ...
    this.platform.log.debug(
      `prepareStream: ${
        this.monitor.monitorConfig.monitor_id
  ...
  async handleStreamRequest(
    ...
    switch (request.type) {
      case StreamRequestTypes.START:
        ...
        this.platform.log.debug(
          `requested video stream: ${width}x${height}, ${fps} fps, ${maxBitrate} kbps, ${mtu} mtu`
        );

        const ffmpegInputArgs =
          this.config.ffmpeg_input_args || "-fflags +genpts";
        const ffmpegProcessArgs =
          this.config.ffmpeg_process_args || "-vsync drop -vcodec copy -an";

        let ffmpegCommand =
          `${ffmpegInputArgs} -i ${this.videoSource} ${ffmpegProcessArgs} ` +
        ...
        const ffmpegProcess = spawn("ffmpeg", ffmpegCommand.split(" "), {
          env: process.env,
        });
      ...
      case StreamRequestTypes.RECONFIGURE:
        // not supported
        this.platform.log.warn(
          `received (unsupported) request to reconfigure to: ${JSON.stringify(
            request.video
          )}`
        );
        callback();
      ...
      case StreamRequestTypes.STOP:
        ...
        this.platform.log.debug(
          `killing: ${this.monitor.monitorConfig.monitor_id} ` +
        ...
        try {
          ...
          this.platform.log.error(
            "error occurred terminating the video process! " + e
          );
        }
  ...
  async shutdown(): Promise<void> {
    Object.keys(this.ongoingSessions).forEach((sessionId) => {
      ...
      this.platform.log.debug(
        `killing: ${this.monitor.monitorConfig.monitor_id} => ${sessionId} => PID: ${ffmpegProcess.pid}`
      );
  ...
  shouldSubStreamBeActive(): boolean {
    const ongoingSessionsSessionsExist =
      Object.keys(this.ongoingSessions).length > 0;
  ...
  getSubStreamIsActive(): Promise<boolean> {
    const url =
      `${this.platform.config.shinobi_api}/${this.platform.config.api_key}/monitor/` +
  ...
  toggleSubStreamActive(subStreamShouldBeActive): Promise<void> {
    const url =
      `${this.platform.config.shinobi_api}/${this.platform.config.api_key}/toggleSubstream/` +
      `${this.platform.config.group_key}/${this.monitor.monitorConfig.monitor_id}`;
    ...
    return fetch(url)
      ...
      .then((shinobiResponse) => {
        this.platform.log.debug(
          `substream toggle response: ${JSON.stringify(shinobiResponse)}`
        );
        if (shinobiResponse.ok !== subStreamShouldBeActive) {
          throw Error(
            `Unable to set substream active state: ${subStreamShouldBeActive}, ` +
      ...
      .catch((err) => {
        this.platform.log.error(
          `toggleSubStreamActive() error: ${err.message}`
        );
      });
  }
  ...
  async setRequiredSubStreamState(): Promise<void> {
    ...
    this.platform.log.debug(
      `subStreamShouldBeActive: ${subStreamShouldBeActive}`
    );
